        ...
        Vue.component('step1', {
            template: `
              <div>
                <div class="form-group" v-bind:class="{ 'has-error': $v.machtigingType }">
                  <label>Ja ik steun Greenpeace{% if ( fields.suggested_frequency ) and not ( fields.allow_frequency_override ) %} <strong>{{ fields.suggested_frequency[1]|lower }}<strong>{% endif %}:</label>

                    <select class="form-control"  id="frequency"
						{% if not ( fields.allow_frequency_override ) %} hidden {% endif %}
					v-model.trim="machtigingType" @input="$v.machtigingType.$touch()">
                      <option value="E">Eenmalig</option>
                      <option value="M">Maandelijks</option>
                      <!-- option value="K">Kwartaal</option>
                      <option value="H">Half jaar</option>
                ...
                <div class="form-row">
                  <div class="form-group col-md-12" v-bind:class="{ 'has-error': $v.bedrag.$error }">
                    ...
                    <div class="radio-list">
                      <input class="form-check-input" v-model.trim="bedrag" type="radio" name="bedrag1" id="bedrag1" value="{% if ( fields.amount1 ) %}{{ fields.amount1 }}{% endif %}">
                      <label class="form-check-label form-control left" for="bedrag1">&euro;{% if ( fields.amount1 ) %}{{ fields.amount1 }}{% endif %}</label>

                      <input class="form-check-input" v-model.trim="bedrag" type="radio" name="bedrag2" id="bedrag2" value="{% if ( fields.amount2 ) %}{{ fields.amount2 }}{% endif %}" checked="checked">
                      <label class="form-check-label form-control" for="bedrag2">&euro;{% if ( fields.amount2 ) %}{{ fields.amount2 }}{% endif %}</label>

                      <input class="form-check-input" v-model.trim="bedrag" type="radio" name="bedrag3" id="bedrag3" value="{% if ( fields.amount3 ) %}{{ fields.amount3 }}{% endif %}">
                      <label class="form-check-label form-control" for="bedrag3">&euro;{% if ( fields.amount3 ) %}{{ fields.amount3 }}{% endif %}</label>
                    </div>
                  </div>
                  ...
                  <div class="form-group col-md-12" v-bind:class="{ 'has-error': $v.bedrag.$error }">
                    ...
                    <div class="input-group">
                      ...
                      <span class="help-block" v-if="$v.bedrag.$error && $v.bedrag.required && $v.bedrag.numeric && !$v.bedrag.between">Het minimale donatiebedrag is {% if ( fields.min_amount ) %}{{ fields.min_amount }}{% endif %} euro</span>
                    </div>
                  </div>
                </div>
              </div>`,
            data() {
                return {
                    machtigingType: '{% if ( fields.suggested_frequency ) %}{{ fields.suggested_frequency[0] }}{% endif %}',
                    bedrag: '{% if ( fields.suggested_amount ) %}{{ fields.suggested_amount }}{% endif %}'
                }
            },
            validations: {
                machtigingType: {
                ...
                bedrag: {
                    required,
                    numeric,
                    between: between({% if ( fields.min_amount ) %}{{ fields.min_amount }}{% endif %}, 999)
    },
        ...
        Vue.component('step2', {
              ...
              <div>
                ...
                <div class="form-group" v-bind:class="{ 'has-error': $v.rekeningnummer.$error }">
                  ...
                  <div class="input-group">
                    ...
                    <span class="help-block" v-if="$v.rekeningnummer.$error && !$v.rekeningnummer.requiredIf">Rekeningnummer is verplicht</span>
                ...
                rekeningnummer: {
                    required : {
                        requiredIf: requiredIf(() => {
                            console.log(donationformVue.ideal);
                            return !donationformVue.ideal;
                        })
                    },
                    alphaNum
                },
            ...
            methods: {
                validate(ideal) {
                    ...
                    if (ideal){
                        console.log("ideal");
                        delete this.$options.validations.rekeningnummer.required;
                    }
                    else{
                        console.log("not ideal");
                        this.$options.validations.rekeningnummer = {required, alphaNum};
                    }
                    // console.log(this.$options.validations.rekeningnummer)
                    this.$v.form.$touch();
                    var isValid = !this.$v.form.$invalid;
                    this.$emit('on-validate', this.$data, isValid);
                    if(isValid){
        ...
        donationformVue = new Vue({
            ...
            data: {
                finalModel: {
                    marketingcode: '{% if ( fields.marketingcode ) %}{{ fields.marketingcode }}{% endif %}',
                    literatuurcode: '{% if ( fields.literatuurcode ) %}{{ fields.literatuurcode }}{% endif %}',
                    guid: ''
                },
                result: {
                    msg: '',
            ...
            methods: {
                ...
                validateStep(name, ideal) {
                    var refToValidate = this.$refs[name];
                    return refToValidate.validate(ideal);
                },
